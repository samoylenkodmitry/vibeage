...
/**
 * PR HH — extra drops appended to existing mob/boss loot tables at
 * the LOOT_TABLES seam. Lives separately so the content-graph
 * validator can keep biome-flavour materials and rare endgame
 * weapons reachable without re-writing each table inline. Each
 * key matches a loot-table id already defined below.
 */
const SUPPLEMENTAL_DROPS: Record<string, readonly LootDrop[]> = {
  // Materials by biome affinity — fills the obtainability gap for
  // every "essence / shard / petal / fragment" that the wiki lists.
  ice_giant_loot: [
