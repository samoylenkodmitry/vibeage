...
| B13 | **rebirth** | "nearly **invulnerable**." | `shield 800` only. | Kept as an 800 shield — "nearly invulnerable" is served by the big absorb; no new effect type needed. | ✅ |
...
| D18 | `iceBolt` poison `value: 0.5` | Bumped to flat 3/tick (DoT ticks are flat; 0.5 was a no-op). | ✅ |
