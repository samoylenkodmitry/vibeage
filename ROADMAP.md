...
4. [ ] **Skill learning bug + per-class starter** — replace the single `DEFAULT_UNLOCKED_SKILLS = ['fireball']` with a per-class starter map (warrior → slash, ranger → arrowShot, healer → holyLight, …). Server emits a typed `LearnSkillFailed` with a reason (`noSkillPoint`, `levelTooLow`, `missingPrereq`, `unknownSkill`); the Skill Tree panel surfaces the rejection inline so it's obvious why a button is greyed out.
