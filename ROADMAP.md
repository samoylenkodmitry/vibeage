...
- [~] Add system messages for level up, death, respawn, loot, learn skill, equip, unequip, and errors. (Combat log + chip channels: level up (`applyPlayerLevelUpFeedback`), death (`applyPlayerDeathFeedback`/`applyEnemyDeathFeedback`), respawn (`applyPlayerRespawnFeedback`), loot acquired (`applyLootAcquiredVisualState`), equip/unequip (`applyEquipmentChangeFeedback`), item used (`applyItemUsedVisualState`), errors via `CommandRejected` routing → `applyCastFailFromCommandRejected` / `applyEquipFailedFromCommandRejected` / `applyInventoryRejectedVisualState` / `applyQuestRejectedVisualState` / `learnSkillRejections` panel chip. Pending: a learn-skill *success* log line (SkillLearned currently only updates state, no combat-log row).)
