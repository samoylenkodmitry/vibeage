...
// Memoized ZoneMarker component
const ZoneMarker = memo<{ zone: any }>(function ZoneMarker({ zone }) {
...
export default function World() {
  ...
  return (
    <>
      ...
      {/* Zone Markers - Memoized components */}
      {GAME_ZONES.map(zone => (
...
// Memoized Lake Water component for misty_lake zone
const LakeWater = memo<{ position: [number, number, number]; radius: number }>(function LakeWater({ position, radius }) {
...
LakeWater.displayName = 'LakeWater';

// Memoized Environmental Objects component for better organization
const EnvironmentalObjects = memo(function EnvironmentalObjects() {
  const environmentalContent = useMemo(() => (
    <group>
        ...
        <group key={`env-${zone.id}`}>
          {zone.id === 'dark_forest' && (
            <>
              ...
              />
              {/* Rare herbalism nodes in the dark forest */}
              <HerbalismNode 
                position={[zone.position.x + 20, 0, zone.position.z + 12]} 
                herbType="shadow_moss" 
              />
              <HerbalismNode 
                position={[zone.position.x - 18, 0, zone.position.z - 8]} 
                herbType="mystical_flower" 
              />
            </>
          )}
          ...
          {zone.id === 'rocky_highlands' && (
            <>
              ...
              />
              {/* Mining nodes for resource gathering */}
              <MiningNode 
                position={[zone.position.x + 10, 0, zone.position.z + 5]} 
                oreType="iron" 
                size={1.2} 
              />
              <MiningNode 
                position={[zone.position.x - 8, 0, zone.position.z - 12]} 
                oreType="crystal" 
                size={1.0} 
              />
            </>
          ...
          {zone.id === 'starter_meadow' && (
            <>
              ...
              <FallenLogs 
                ...
                spread={zone.radius * 0.6} 
              />
              {/* Herbalism nodes for beginners */}
              <HerbalismNode 
                position={[zone.position.x + 15, 0, zone.position.z + 8]} 
                herbType="common_herb" 
              />
              <HerbalismNode 
                position={[zone.position.x - 10, 0, zone.position.z - 15]} 
                herbType="common_herb" 
              />
            </>
          )}
          ...
          {zone.id === 'volcanic_wastes' && (
            <>
              ...
              />
              {/* Rare mining nodes in volcanic area */}
              <MiningNode 
                position={[zone.position.x + 25, 0, zone.position.z + 18]} 
                oreType="mithril" 
                size={1.5} 
              />
              <MiningNode 
                position={[zone.position.x - 20, 0, zone.position.z - 15]} 
                oreType="gold" 
                size={1.3} 
              />
            </>
          ...
          {zone.id === 'ethereal_gardens' && (
            <>
              ...
              <MagicalTrees 
                ...
                spread={zone.radius * 0.9} 
              />
              {/* Mystical herbalism and rune stones */}
              <HerbalismNode 
                position={[zone.position.x + 12, 0, zone.position.z + 20]} 
                herbType="mystical_flower" 
              />
              <HerbalismNode 
                position={[zone.position.x - 15, 0, zone.position.z - 18]} 
                herbType="fire_blossom" 
              />
              <RuneStone 
                position={[zone.position.x + 8, 0, zone.position.z - 25]} 
                runeType="ethereal" 
              />
              <RuneStone 
                position={[zone.position.x - 22, 0, zone.position.z + 10]} 
                runeType="nature" 
              />
            </>
          )}
          ...
          {zone.id === 'cursed_ruins' && (
            <>
              ...
              <BoulderField 
                ...
                spread={zone.radius * 0.7} 
              />
              {/* Ancient rune stones and dungeon portal */}
              <RuneStone 
                position={[zone.position.x + 18, 0, zone.position.z + 14]} 
                runeType="shadow" 
              />
              <RuneStone 
                position={[zone.position.x - 12, 0, zone.position.z - 20]} 
              ...
              />
              <DungeonPortal 
                position={[zone.position.x + 5, 0, zone.position.z - 8]} 
          ...
          {zone.id === 'temporal_rifts' && (
            <>
              ...
              <DistortedTerrain 
                ...
                spread={zone.radius * 0.8} 
              />
              {/* Temporal rune stones */}
              <RuneStone 
                position={[zone.position.x + 16, 0, zone.position.z + 22]} 
                runeType="temporal" 
              />
              <RuneStone 
                position={[zone.position.x - 18, 0, zone.position.z - 12]} 
                runeType="void" 
              />
            </>
          )}
          ...
          {zone.id === 'abyssal_depths' && (
            <>
              ...
              <DarkCorals 
                ...
                spread={zone.radius * 0.8} 
              />
              {/* Deep sea dungeon portal */}
              <DungeonPortal 
                position={[zone.position.x - 15, 0, zone.position.z + 18]} 
                dungeonId="abyssal_trench" 
                size={2.0} 
              />
            </>
          )}
          ...
          {zone.id === 'celestial_peaks' && (
            <>
              <CrystalFormations 
                position={[zone.position.x, 0, zone.position.z]} 
                count={15} 
                spread={zone.radius * 0.5} 
              />
              <LightPillars 
                position={[zone.position.x, 0, zone.position.z]} 
                count={5} 
                spread={zone.radius * 0.3} 
              />
            </>
          )}
        </group>
...
EnvironmentalObjects.displayName = 'EnvironmentalObjects';
...
// Simple tree component - Made slightly larger - Memoized
const Tree = memo<{ position?: [number, number, number]; scale?: number }>(function Tree({ position = [0, 0, 0], scale = 1 }) {
