...
describe('dispelTargetSet — static category → effect-type map', () => {
  it('negative covers the pre-§52 fixed set', () => {
    ...
    for (const type of ['slow', 'stun', 'burn', 'poison', 'dot', 'freeze', 'timeStop', 'waterWeakness', 'marked']) {
  ...
  it('positive covers every purgeable buff', () => {
    ...
    for (const type of ['heal', 'shield', 'damageReflect', 'bless', 'arcaneCharge', 'evasion', 'invisible', 'speed_boost', 'attackSpeed', 'reveal_loot', 'invuln']) {
      expect(set.has(type)).toBe(true);
    }
...
describe('resolveCastImpact — category-aware dispel end-to-end', () => {
  it("default ('negative') strips the same fixed set as pre-§52 (regression net)", () => {
    expect(runDispelWith(undefined, ['slow', 'stun', 'timeStop', 'burn', 'marked', 'bless', 'heal'])).toEqual(['bless', 'heal']);
  });
  it("'positive' strips only buffs (anti-buff purge)", () => {
    expect(runDispelWith('positive', ['slow', 'bless', 'damageReflect', 'arcaneCharge', 'shield', 'evasion', 'attackSpeed'])).toEqual(['slow']);
  });
  ...
  it("'stun' strips action-blockers, leaves slow + buffs untouched", () => {
    expect(runDispelWith('stun', ['stun', 'freeze', 'timeStop', 'slow', 'bless'])).toEqual(['bless', 'slow']);
  });
