...
describe('water-weakness damage amplifier', () => {
  it('amplifies a water cast by 1 + value/100 when the target has waterWeakness', () => {
    ...
    resolveCastImpact(cast('waterSplash', caster, enemy), { publish: (e) => eBase.push(e) }, makeWorld(caster, enemy), 1_000);
    ...
    resolveCastImpact(cast('waterSplash', caster, enemy), { publish: (e) => eAmp.push(e) }, makeWorld(caster, enemy), 1_000);
  ...
  it('does NOT amplify a non-water cast on a waterWeakness target', () => {
    ...
    const eBase: OutboundEvent[] = [];
    resolveCastImpact(cast('fireball', caster, enemy), { publish: (e) => eBase.push(e) }, makeWorld(caster, enemy), 1_000);
    const base = damageFromCombatLog(eBase);

    enemy.health = enemy.maxHealth;
    applyWaterWeakness(enemy, 30, 5_000);
    const eWithEffect: OutboundEvent[] = [];
    resolveCastImpact(cast('fireball', caster, enemy), { publish: (e) => eWithEffect.push(e) }, makeWorld(caster, enemy), 1_000);
